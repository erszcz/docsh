...
-record(docsh_beam, {abst, beam_file, source_file}).

-opaque t() :: #docsh_beam{%% Abst chunk which may or may not be present in a loaded module.
...
from_loadable_module(Mod) ->
    case code:which(Mod) of
        ...
        BEAMFile ->
            {ok, #docsh_beam{abst = debug_info(BEAMFile),
                             beam_file = BEAMFile,
